...
from utils import data_to_squad_format
...
def parse_args():
    ...
    parser.add_argument(
        "--test_file", type=str, default='./dataset/test_relevant.json', help="A csv or a json file containing the Prediction data."
    )
    parser.add_argument(
        "--context_file", type=str, default='./dataset/context.json', help="A csv or a json file containing the paragraph context data."
    )
    parser.add_argument(
        "--prediction_file", type=str, default='./prediction.csv', help="A csv or a json file containing the paragraph context data."
    )
    parser.add_argument(
...
def main():
    ...
    raw_datasets = raw_datasets.map(data_to_squad_format, batched=True, fn_kwargs={'context':context}, load_from_cache_file=False)
    ...
    if args.do_predict:
        ...
        prediction = pd.DataFrame(prediction).rename(columns={'prediction_text':'answer'})
        # print(prediction)
        if args.output_dir is not None:
            prediction.to_csv(args.prediction_file, index=False)
