...
def main():
    ...
    if args.dataset_name is not None:
        ...
        logger.info(f'Step2: Load {len(raw_datasets["train"])} training instances and {len(raw_datasets["validation"])} validating instances')
    ...
    logger.info(f'Step3: Model {args.model_name_or_path} loaded')
    ...
    logger.info(f'Step4. Data preprocessed and tokenized.')
    ...
    logger.info(f'Step5. DataLoader created')

    '''
    6. Initialize optimizer and training scheduler
    '''
    # Optimizer
    ...
    logger.info(f'Step5. Optimizer and schedular initiated.')
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(tqdm(eval_dataloader)):
            with torch.no_grad():
                ...
                total_decoded_labels.extend(decoded_labels)
                metric.add_batch(
        ...
        if result['rouge1'] > best_rougeL:
            best_rougeL = result['rouge1']

            with open(os.path.join(args.output_dir, 'decoded_preds.jsonl'), 'w') as file:
                for preds, id in zip(total_decoded_preds, raw_datasets['validation']['id']):
                    record = {}
                    record['title'] = preds
                    record['id'] = id
            ...
            with open(os.path.join(args.output_dir, 'decoded_labels.jsonl'), 'w') as file:
                for preds, id in zip(total_decoded_labels, raw_datasets['validation']['id']):
                    record = {}
                    record['title'] = preds
                    record['id'] = id
                    json.dump(record, file)
